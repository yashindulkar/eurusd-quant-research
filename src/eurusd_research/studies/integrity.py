...
class Task03RowMembershipEvidence(StrictModel):
    """Layered Task 03 evidence consumed by Task 04 v2.10."""
...
def _source_scope(root: Path) -> list[tuple[Path, bool, str]]:
    ...
    files.append(
        (
            root / "studies" / "task04_v2.10_environment_lock.json",
            False,
            "environment_lock",
        )
    )
...
def validate_source_dependency_manifest(
    ...
    path = root / "studies" / "task04_v2.10_source_manifest.json"
...
def validate_task03_row_membership(
    ...
    *,
    ...
    path = root / "studies" / "task03_task04_v2.10_evidence.json"
