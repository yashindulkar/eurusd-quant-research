"""Prepare pre-result v2.3 dependency evidence without Task 04 calculation."""
...
def _environment_lock() -> dict[str, object]:
    return {
        ...
        "policy": (
            "Exact versions used for the registered v2.3 regeneration. The "
            "package constraints remain in pyproject.toml; this lock is receipt-bound."
        ),
    }


def main() -> int:
    """Write deterministic pre-anchor evidence and print bounded fingerprints."""
    ...
    task03_path = root / "studies" / "task03_task04_v2.3_mask_evidence.json"
    _atomic_json(task03_path, task03_evidence.model_dump(mode="json"))

    source_manifest = build_source_dependency_manifest(root)
    source_path = root / "studies" / "task04_v2.3_source_manifest.json"
    ...
    print(
        "Task 04 v2.3 pre-anchor evidence: PREPARED | "
