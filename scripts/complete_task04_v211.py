"""Promote validated v2.11 candidate outputs and create the terminal lifecycle."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    standalone_identity = read_validated_candidate_identity(
        project_path(config.validated_candidate_identity_path, root)
    )
    standalone_reconciliation = read_independent_reconciliation(
        project_path(config.independent_reconciliation_path, root)
    )
    if request.validated_candidate_identity != standalone_identity:
        raise ValueError("Completion request substituted candidate identity evidence")
    if request.independent_reconciliation != standalone_reconciliation:
        raise ValueError("Completion request substituted reconciliation evidence")
    ...
    print(
        "Task 04 v2.11 lifecycle: COMPLETED | "
