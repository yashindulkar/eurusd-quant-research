...
def main() -> int:
    ...
    if project_path(config.registration_receipt_path, root).exists():
        raise ValueError("v2.3 receipt already exists; this is not pre-anchor state")
    if project_path(config.registration_lifecycle_path, root).exists():
        raise ValueError("v2.3 lifecycle already exists before anchoring")
    if project_path(config.output_directory, root).exists():
        raise ValueError("v2.3 production output directory exists before anchoring")
    print(
