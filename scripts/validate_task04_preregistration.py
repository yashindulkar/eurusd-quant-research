...
def main() -> int:
    ...
    if project_path(config.registration_receipt_path, root).exists():
        raise ValueError("v2.4 receipt already exists; this is not pre-anchor state")
    if project_path(config.registration_lifecycle_path, root).exists():
        raise ValueError("v2.4 lifecycle already exists before anchoring")
    if project_path(config.output_directory, root).exists():
        raise ValueError("v2.4 production output directory exists before anchoring")
    if project_path(config.candidate_output_directory, root).exists():
        raise ValueError("v2.4 candidate output directory exists before anchoring")
    print(
